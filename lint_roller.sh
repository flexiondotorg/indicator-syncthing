...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive indicator_syncthing/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive indicator_syncthing/


  echo "Running flake8"

    >&2 flake8 indicator_syncthing/
